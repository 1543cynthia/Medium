...
// 刻度顯示格式轉換 套用formatTicks在大數字上
function formatTicks(d) {
    // r四捨五入指定的小數精度位數 s以參數 r 轉換後的格式為基礎，但帶有一個單位碼
    // ('.取幾位數') ('~s') 只要在參數前面加個 ~ 就可以刪掉那些多餘的 0 了 
    //不過這個 ~ 只適用在 e 、 r 、 % 以及 s 這四種參數上喔！
    
    return d3.format('~s')(d)//d3.format() 轉換後的數值，其型態會是字串而非原本的數字
